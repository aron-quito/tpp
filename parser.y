...
%type <nodo> programa lista_elementos elemento declaracion_fun parametros lista_parametros parametro declaracion_var lista_ids declarador bloque_instrucciones instruccion estructura_si estructura_mientras estructura_para estructura_depende lista_casos caso valor_constante lista_impresion elemento_impresion expresion llamada_funcion argumentos lista_argumentos
...
lista_ids:
    lista_ids COMA declarador { $$ = nuevo_nodo_lista_elementos($1, $3); }
    | declarador              { $$ = $1; }
    ;
...
estructura_si:
    SI PARI expresion PARD LLAVEI bloque_instrucciones LLAVED %prec LOWER_THAN_SINO { $$ = nuevo_nodo_si($3, $6, NULL); }
    | SI PARI expresion PARD LLAVEI bloque_instrucciones LLAVED SINO LLAVEI bloque_instrucciones LLAVED { $$ = nuevo_nodo_si($3, $6, $10); }
...
estructura_mientras:
    MIENTRAS PARI expresion PARD LLAVEI bloque_instrucciones LLAVED { $$ = nuevo_nodo_mientras($3, $6); }
...
estructura_depende:
    DEPENDE expresion LLAVEI lista_casos LLAVED { $$ = nuevo_nodo_depende($2, $4); }
