...
/* Definimos una lista con todos los tipos de nodos posibles que puede tener el
 * árbol */
typedef enum {
  N_ENTERO,
  N_DECIMAL,
...
typedef struct NodoAST {
  ...
  struct NodoAST *adicional; /* Para el cuerpo de estructuras complejas (PARA) o
                                más ramas */

  int operador; /* Guardará el token de la operación (MAS, MENOS, MULT...) o
                   tipo */
