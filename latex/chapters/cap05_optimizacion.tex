...
En la arquitectura del compilador TPP (contrario a la mayoría de compiladores modernos), la \textbf{optimización del AST se ejecuta estrictamente antes que la generación del Código Intermedio (TAC)}. Esta decisión de diseño se fundamenta en tres razones principales:
...
\begin{tikzpicture}[
    ...
    node/.style={rectangle, rounded corners=4pt, draw=tppblue, fill=tppblue!12,
                 minimum width=1.5cm, minimum height=0.6cm, font=\small\bfseries, thick},
    leaf/.style={rectangle, rounded corners=4pt, draw=tppgreen!80!black, fill=tppgreen!10,
                 minimum width=1.2cm, minimum height=0.55cm, font=\small}
]
% AST Original (Izquierda)
\node[font=\small\bfseries, text=tppdark] at (0, 0.8) {Antes (AST)};
\node[node] (op1) at (0, 0) {\texttt{+}};
\node[leaf] (l3)  at (-1, -1.2) {\texttt{3}};
\node[leaf] (l5)  at (1, -1.2) {\texttt{5}};
\draw[->, tppblue, thick] (op1)--(l3);
\draw[->, tppblue, thick] (op1)--(l5);

% Flecha de transformación
\draw[->, thick, tppred, decorate, decoration={snake, amplitude=.4mm, segment length=2mm, post length=2mm}] (2.5, -0.6) -- (5.5, -0.6) node[midway, above, font=\footnotesize, text=tppdark] {Plegado};

% AST Optimizado (Derecha)
\node[font=\small\bfseries, text=tppdark] at (8, 0.8) {Después (Optimizado)};
\node[leaf] (opt_node)  at (8, -0.6) {\texttt{8}};

\end{tikzpicture}
\caption{Visualización del Plegado de Constantes. El subárbol de la operación constante (\texttt{3 + 5}) colapsa en un solo nodo hoja constante (\texttt{8}).}
\label{fig:opt_fold}
\end{figure}
...
\node[font=\small\bfseries, text=tppdark] at (0, 0.8) {Antes (AST)};
\node[node] (op1) at (0, 0) {\texttt{+}};
\node[leaf] (lvar)  at (-1, -1.2) {\texttt{x}};
\node[leaf] (l0)  at (1, -1.2) {\texttt{0}};
\draw[->, tppblue, thick] (op1)--(lvar);
\draw[->, tppblue, thick] (op1)--(l0);

% Flecha de transformación
\draw[->, thick, tppred, decorate, decoration={snake, amplitude=.4mm, segment length=2mm, post length=2mm}] (2.5, -0.6) -- (5.5, -0.6) node[midway, above, font=\footnotesize, text=tppdark] {Simplificación};

% AST Optimizado (Derecha)
\node[font=\small\bfseries, text=tppdark] at (8, 0.8) {Después (Optimizado)};
\node[leaf] (opt_node)  at (8, -0.6) {\texttt{x}};

\end{tikzpicture}
\caption{Visualización de la Simplificación Algebraica. La operación de suma con el elemento neutro $0$ (\texttt{x + 0}) se elimina, promoviendo el nodo de la variable (\texttt{x}) para reemplazar a su padre.}
\label{fig:opt_algebraic}
\end{figure}
